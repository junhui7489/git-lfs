...
func (p *ProgressMeter) Updates() <-chan *log.Update { return p.updates }
func (p *ProgressMeter) Throttled() bool             { return true }

func (p *ProgressMeter) update() {
	if p.dryRun || (p.estimatedFiles == 0 && p.skippedFiles == 0) || atomic.LoadInt32(&p.started) == 0 {
		return
	}
...
func (p *ProgressMeter) msg() string {
	...
	out += fmt.Sprintf(") %s / %s", humanize.FormatBytes(uint64(p.currentBytes)), humanize.FormatBytes(uint64(p.estimatedBytes)))
	if p.skippedBytes > 0 {
		out += fmt.Sprintf(", %s skipped", humanize.FormatBytes(uint64(p.skippedBytes)))
	}
